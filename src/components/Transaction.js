...
function Transaction() {
  ...
  const Pending = () => {
    return (
      <div>
        <span className="txId">
          <a href={`https://testnet.flowscan.org/transaction/${txId}`}>{txId?.slice(0, 8)}</a>
        </span>
        <span>
          <kbd>Pending</kbd>
          <br />
    ...
    return (
      <div>
        ...
        <span>
          ...
          <br />
          <small>
            The verification nodes have verified the transaction, and the seal is included in the
            latest block.
          </small>
        </span>
